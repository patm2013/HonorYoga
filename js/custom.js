...
var makeCalendar = function(events){ 
  ...
  $('#cal').clndr({
    ...
    clickEvents: {
      ...
      click: function(target) {
        ...
        if(target.events.length) {
          ...
          var templateData = {events: target.events};
    ...
    ready: function(){
        ...
        var now = moment();
        var todaysEvents = _.filter(events, function(event){ return moment(event.date).isSame(now, "day"); });
        console.log(todaysEvents);
        var templateData = {events: todaysEvents};
        template = _.template($("#event-template").html());
          $("#event").html(template(templateData));
      }
  });
...
var addUpcomingEvents = function(upcomingEvents) {
  ...
  var templateData = {events: shortList};
